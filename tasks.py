...
# @task
# def install(c):
#     dist_path = os.path.join(DIST_DIR, "escapefromtarkov_" + MANIFEST['guid'])
#     build(c, output=dist_path)
#
#
# @task
# def pack(c):
#     output = "escapefromtarkov_" + MANIFEST['guid']
#     build(c, output=output, ziparchive='escapefromtarkov_v{}.zip'.format(MANIFEST['version']))
#     rmtree(output)